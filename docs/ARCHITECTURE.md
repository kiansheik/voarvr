...
## Avian morphology and motion evidence

Magpie opts into `WingArchitecture.ArticulatedAvian` with optional managed-reference `BirdMorphology`; Duck retains legacy tuning and Dragon is explicitly membrane/fantasy. Biological/reference mass and area are separate from2.5× Magpie embodiment scale and human calibration. `AvianWingPresentation:IWingArticulation` configures bounded generic shoulder controls, then couples manus/primary/secondary/alula/tail states after core IK. Folded articulation nests along the flank; no bone scaling occurs. Point-mass tail drag/heading and alula stall delay are explicit approximations, not per-feather aerodynamics. Future membrane/insect systems need their own capabilities, not avian feather constraints.

Development-only local telemetry copies raw device and mapped frames into a bounded background writer. Calibration acceptance sequence and actual pre-step input gate are recorded separately, allowing simple replay to reproduce Meta acceptance frames correctly. World/force/rig/performance state is captured without per-frame string serialization. See [telemetry contract](development/telemetry.md) and [research provenance](research/species/magpie.md). Personal captures remain ignored; no runtime MCP or cloud dependency.

## Flight Game v1

`Flight/AcrobaticFlight` contains optional pure rotational dynamics and trick detection. Controller preserves a separate Beginner branch. `Gameplay/FlightChallenge` consumes observations in deterministic logical coordinates; `ExpeditionDirector` bridges runtime/UI/local best persistence. `World/FlightRegions` provides altitude/weather geography and persistent route convection alongside finite AtmosphereModel plumes. `SkyArchipelago` reuses nine logical slots and near/far combined geometry; `SkywardKit` caches baked authored mesh data. `SkywardSetup.Configure` explicitly imports FBX transforms into catalog subassets through the Editor API. Telemetry container v2 adds275 scalar fields total and appended event IDs, while reader accepts known v1 fields. Offline replay still needs matching external wind/collision environment for world equivalence.
