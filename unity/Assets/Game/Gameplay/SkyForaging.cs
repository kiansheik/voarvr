...
namespace VoarVR.Gameplay
{
    ...
    public sealed class SkyForaging:MonoBehaviour
    {
        ...
        private const string BestKey="VoarVR.ForagingBest.v1";
        ...
        {
            bird=driver;space=coordinates;wind=field;best=PlayerPrefs.GetInt(BestKey,0);
        ...
        public void InvalidateSweep(){previousValid=false;}
        public void Tick(float dt)
        {
            ...
            for(int i=0;i<Capacity;i++)
            {
                ...
                if(visible && canCatch && !reset && ForagingScore.Touches(previous,p,food,2.2f))
            ...
            previous=p;mesh.vertices=vertices;mesh.normals=normals;mesh.RecalculateBounds();
        ...
        private void OnDestroy(){if(mesh!=null)Destroy(mesh);if(counter!=null)Destroy(counter.gameObject);if(Score.Points>PlayerPrefs.GetInt(BestKey,0)){PlayerPrefs.SetInt(BestKey,Score.Points);PlayerPrefs.Save();}}
    }
}
